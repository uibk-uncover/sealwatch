...
# from .constants import JRM, CC_JRM, GFR, PHARM_ORIGINAL, PHARM_REVISITED, SPAM
...
def generate_groups(feature_type):
    ...
    with NamedTemporaryFile(suffix=".jpeg") as f:
        ...
        if Features.FEATURES_JRM == feature_type:
            dummy_features = jrm.extract_from_file(f.name)

        elif Features.FEATURES_CCJRM == feature_type:
            dummy_features = jrm.extract_from_file(f.name, calibrated=True)

        elif Features.FEATURES_GFR == feature_type:
            dummy_features = gfr.extract_from_file(f.name, num_rotations=32, qf=75)

        elif Features.FEATURES_PHARM == feature_type:
            dummy_features = pharm.extract_from_file(f.name)

        elif Features.FEATURES_SPAM == feature_type:
            dummy_features = spam.extract_from_file(f.name)
