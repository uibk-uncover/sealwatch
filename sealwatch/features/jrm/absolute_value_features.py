...
def compute_absolute_value_features(abs_X, T=3):
    ...
    append_features(features, Ah, prefix=f"Ah_T{T}")
    ...
    append_features(features, Ad_diag, prefix=f"Ad_T{T}_diag")

    # { C(x, y, 1, -1) | 0 <= x <= y; x + y <= 5}
    mode_ids = [6, 7, 8, 9, 10, 12, 13, 14, 17]
    Ad_semidiag = extract_submodels_abs(abs_X, mode_ids, T=T, direction="semidiag")
    append_features(features, Ad_semidiag, prefix=f"Ad_T{T}_semidiag")
    ...
    append_features(features, Aoh, prefix=f"Aoh_T{T}")
    ...
    append_features(features, Ax, prefix=f"Ax_T{T}")
    ...
    append_features(features, Aod_diag_skip, prefix=f"Aod_T{T}_diag_skip")
    ...
    append_features(features, Aod_semidiag_skip, prefix=f"Aod_T{T}_semidiag_skip")
    ...
    append_features(features, Am, prefix=f"Am_T{T}")
    ...
    append_features(features, Aih, prefix=f"Aih_T{T}")
    ...
    append_features(features, Aid, prefix=f"Aid_T{T}")
    ...
    append_features(features, Ais, prefix=f"Ais_T{T}")
    ...
    append_features(features, Aix, prefix=f"Aix_T{T}")
